...
<div class="col-md-10">
  			...
  			<div class="content-box-large">
				...
				<div class="row">
					<div class="col-md-8">
						<a href="${pageContext.request.contextPath}/admin/land/add" class="btn btn-success">
				...
				<div class="row">
  				<div class="panel-body">
					...
					<c:if test="${not empty totalPage }">
						<c:if test="${not empty curPage && not empty searchLand}">
							<c:if test="${totalPage > 1}">
							<nav class="text-center" aria-label="...">
							   <ul class="pagination">
							   	<c:if test="${curPage > 1}">
							   		<li><a href="${pageContext.request.contextPath}/admin/land/search?ql=${searchLand}" aria-label="Previous"><span aria-hidden="true">«</span></a></li>
							   	</c:if>
							   	<c:forEach var="i" begin="1" end="${totalPage}">
							   		<c:choose>
							   			<c:when test="${curPage == i}">
							   			 <li class="active"><a href="${pageContext.request.contextPath}/admin/land/search/${i}?ql=${searchLand}">${i} <span class="sr-only">(current)</span></a></li>
							   			</c:when>
							   			<c:otherwise>
							   			<li><a href="${pageContext.request.contextPath}/admin/land/search/${i}?ql=${searchLand}">${i}</a></li>
							   			</c:otherwise>
							   		</c:choose>
							   	</c:forEach>
							    <c:if test="${curPage < totalPage}">
							    	<li><a href="${pageContext.request.contextPath}/admin/land/search/${totalPage}?ql=${searchLand}" aria-label="Next"><span aria-hidden="true">»</span></a></li>
							    </c:if>
							   </ul>
							</nav>
							</c:if>
